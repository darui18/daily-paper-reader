  ...
  plugins: [
    function (hook, vm) {
        ...
        try {
          ...
          if (sectionEl) {
            ...
            const fallbackGlancePairs = collectLabeledPairs([
              ...fallbackArray(frontmatterPaperMeta.problem_solution, '问题与解决方式'),
            ]);
            ['问题与解决方式'].forEach((label) => {
              const key = 'problem_solution';
      ...
      const renderPaperFromMeta = (meta) => {
        ...
        // 速览区域
        if (meta.problem_solution) {
          lines.push('<div class="paper-glance-section">');
          lines.push('<div class="paper-glance-row">');

          lines.push('<div class="paper-glance-col">');
          lines.push('<div class="paper-glance-label">问题和解决方式</div>');
          lines.push(`<div class="paper-glance-content">${escapeHtml(meta.problem_solution)}</div>`);
          lines.push('</div>');
