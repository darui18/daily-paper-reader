...
def normalize_glance_block_format(md_text: str) -> Tuple[str, bool]:
    ...
    for line in lines:
        ...
        if in_glance:
            ...
            if stripped.startswith("**简介和核心思想**：") or stripped.startswith("**简介和核心思想**:"):
                new_line = ensure_line_break(line)
            elif stripped.startswith("**问题和解决方式**：") or stripped.startswith("**问题和解决方式**:"):
...
def generate_deep_summary(
    ...
    system_prompt = (
        "你是一名科普写作者，擅长把机器人、CV、ML 论文翻译成工程团队能秒懂的通俗语言。"
        "读者是 VLA / 移动操作方向的算法工程师。\n\n"
        "写作原则：\n"
        "- 通用术语正常用（VLA、世界模型、扩散策略、端到端、Token化、Sim2Real 等），不需要解释\n"
        "- 上下文依赖的黑话禁止使用，必须拆成具体描述。"
        "比如"底层策略"→说清楚是"负责电机/关节控制动作的模块"；"高层规划器"→说清楚是"负责分解子任务的模块"\n"
        "- 打比方：抽象概念用生活化比喻。如"世界模型"→"在脑子里预演动作结果，像下棋时算几步"\n"
        "- 有具体数字的用数字说话，同时补充直观感受（如"比以前快了近一倍"）\n"
        "- 用中文、Markdown 形式"
    )
    user_prompt = (
        "请基于下面提供的论文内容，写一篇深度解读。每点的长度控制在合理范围，不要啰嗦。\n\n"
        "## 0. 为什么值得看\n"
        "一句话说清楚：这篇论文跟你的研究方向有什么直接关联？是打开了一种新思路，还是解决了一个卡了很久的具体问题，还是方法能直接拿来用？\n\n"
        "## 1. 这篇论文做了什么\n"
        "用大白话讲：交代背景（大家之前在纠结什么）、核心想法（作者换了个什么角度来破局）。"
        "不需要复述论文摘要，而是用你自己的话说清楚：如果只能选一件事讲给别人听，这件事是什么。\n\n"
        "## 2. 现有方法有什么毛病\n"
        "写清楚作者发现了什么痛点，为什么之前的方法绕不开这个问题。"
        "如果能有具体的失败场景描述（比如"机器人在堆满杂物的桌子前会愣住"）会更好。\n\n"
        "## 3. 怎么解决的\n"
        "讲清楚方案的三个关键设计就够了：核心思路是什么、怎么实现的、跟原来的区别在哪。"
        "可以用"原来→现在"的对比方式。不要罗列所有模块，选最重要的 2-3 个讲透即可。\n\n"
        "## 4. 效果怎么样\n"
        "用具体数字说话：在什么场景/数据集上测的，比最好的对手强多少（具体百分比或绝对数值），"
        "哪些情况提升最明显。如果有失败case也提一句，这比全夸更可信。\n\n"
        "## 5. 接下来可以做什么\n"
        "结合 VLA / 移动操作机器人方向提出 2-3 个具体切入点：这篇方法能不能移植到你的场景里？"
        "有哪些假设在当前条件下可能不成立？作者没讲清楚的地方是什么？\n\n"
        "要求：最后单独输出一行"（完）""。
    )
...
def generate_glance_overview(
    ...
    """
    生成论文速览（包含 简介和核心思想、问题和解决方式）。
    使用 JSON 结构化输出，确保返回完整的两个字段。
    """
    active_client = client or LLM_CLIENT
    if active_client is None:
    ...
    system_prompt = (
        "你是论文速览助手。用最通俗的中文把论文概括成两段话，让其他方向的机器人工程师也能一眼看懂。"
        "VLA、世界模型、扩散策略、端到端等通用术语正常用；"
        "但"底层策略""高层规划器"这类空洞词要拆成具体描述（"控制电机的模块""分解子任务的模块"）。"
        "保留英文模型名，生僻术语用括号解释。"
    )
    payload = {"title": title, "abstract": abstract}
    user_text = json.dumps(payload, ensure_ascii=False)
    user_prompt = (
        ...
        "{\"summary\":\"...\",\"problem_solution\":\"...\"}\n"
        "要求：\n"
        "- summary：简介和核心思想（100-180个中文字符）。用"这篇文章要解决……，核心思路是……"的方式写，不要堆砌术语\n"
        "- problem_solution：问题和解决方式（80-150个中文字符）。用"以前的难点在于……，这篇文章的办法是……"的对比方式\n"
        "- 如果有具体数字（如提升X%、达到Y%成功率），一定要写进去\n"
        "- 不要把英文句子放进中文字段；可保留必要英文术语或模型名，但必须用括号解释\n"
        "Output must be strict JSON only, no markdown, no fences, no extra text."
    )
    ...
    schema = {
        ...
        "required": ["summary", "problem_solution"],
        "additionalProperties": False,
    }
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            problem_solution = str(obj.get("problem_solution") or "").strip()
            if not (summary and problem_solution):
                continue
            return "\n".join(
                [
                    f"**简介和核心思想**：{ensure_single_sentence_end(summary)} \\",
                    f"**问题和解决方式**：{ensure_single_sentence_end(problem_solution)}",
                ]
            )
        except Exception as e:
            # 额度不足等“硬失败”不必重试，直接降级
...
def build_glance_fallback(paper: Dict[str, Any]) -> str:
    ...
    - 简介和核心思想 优先用 llm_tldr_cn/llm_tldr；否则用摘要首句；
    - 问题和解决方式 用基于摘要的启发式生成，保证 2 段齐全。
    """
    ...
    if not tldr and evidence:
        tldr = evidence
    summary = ensure_single_sentence_end(tldr or "基于摘要生成的速览信息。")

    problem_solution = first_sentence(evidence)
    if not problem_solution and abstract:
        m = re.search(r"(we (?:propose|present|introduce|develop)[^\.]{0,200})\.", abstract, re.I)
        if m:
            problem_solution = m.group(1).strip()
    if not problem_solution:
        problem_solution = "本文针对现有方法的不足，提出了新的方法并在相应任务上进行了验证。"
    problem_solution = ensure_single_sentence_end(problem_solution)

    return "\n".join(
        [
            f"**简介和核心思想**：{summary} \\",
            f"**问题和解决方式**：{problem_solution}",
        ]
    )
...
def build_markdown_content(
    ...
    glance_summary = ""
    glance_problem_solution = ""

    if glance:
        for line in glance.split("\n"):
            line = line.strip().rstrip("\\").strip()
            if line.startswith("**简介和核心思想**：") or line.startswith("**简介和核心思想**:"):
                glance_summary = line.split("：", 1)[-1].split(":", 1)[-1].strip()
            elif line.startswith("**问题和解决方式**：") or line.startswith("**问题和解决方式**:"):
                glance_problem_solution = line.split("：", 1)[-1].split(":", 1)[-1].strip()

    # 优先使用速览生成的简介和核心思想，否则使用原来的 TLDR
    display_tldr = glance_summary if glance_summary else tldr
    ...
    # 速览字段
    if glance_problem_solution:
        lines.append(f"problem_solution: {yaml_escape_value(glance_problem_solution)}")
