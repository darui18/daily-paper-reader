...
def generate_deep_summary(
    ...
    system_prompt = (
        "你是一名资深学术论文分析助手，尤其擅长机器人、计算机视觉和机器学习领域。"
        "请使用中文、以 Markdown 形式，对给定论文做结构化、深入、客观的总结。"
    )
    user_prompt = (
        "请基于下面提供的论文内容，按照如下结构生成一篇关于该论文的深度解读。\n"
        "每一点的输出长度控制在合理范围内，不要过于冗长。\n\n"
        "## 0. 为什么推荐精读\n"
        "说明这篇论文为什么值得精读：是开创性工作、解决了关键瓶颈、方法有普适性，还是与你的研究方向高度相关。\n\n"
        "## 1. 论文简介和核心思想\n"
        "简要介绍论文的背景、要解决的核心问题，以及方法的核心思想。用一两句话说清楚论文做了什么。\n\n"
        "## 2. 论文发现的问题\n"
        "指出现有方法存在的不足和局限性，以及论文作者观察到的关键问题。\n\n"
        "## 3. 解决方案\n"
        "论文提出的具体方法、技术细节、创新点。包括模型架构、训练策略、关键设计等。\n\n"
        "## 4. 效果\n"
        "实验设置（数据集、场景、baseline）、关键定量结果、定性分析、消融实验结论。\n\n"
        "## 5. 下一步计划\n"
        "基于这篇论文可以继续探索的方向、可以结合到自己工作的思路、值得关注的后续工作。\n\n"
...
def generate_glance_overview(
    ...
    system_prompt = "你是论文速览助手，请用中文生成简洁但信息完整的论文速览。"
    payload = {"title": title, "abstract": abstract}
    user_text = json.dumps(payload, ensure_ascii=False)
    user_prompt = (
        ...
        "要求：\n"
        "- summary：论文的简介和核心思想（100-180个中文字符），说明论文要解决什么问题、核心方法是什么\n"
        "- problem_solution：论文发现的问题和解决方式（80-150个中文字符），指出现有方法的不足和论文的解决方案\n"
