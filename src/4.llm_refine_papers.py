...
def call_filter(
    ...
    chinese_style_guide = (
        ...
        "\n可正常使用的通用术语：VLA、世界模型、扩散策略、端到端、Token化、行为克隆、"
        "动作分块、真机实验、仿真迁移、Sim2Real。这些领域通用词不需要额外解释。\n"
        "\n必须避免的空洞黑话："
        "不要用[底层策略][高层规划器][多级推理层次][任务级表征][跨模态对齐]这类没有上下文根本看不懂的词。"
        "要写具体的 -- [底层策略]写清楚是[负责控制电机/关节动作的那个模块]，"
        "[高层规划器]写清楚是[负责把[拿杯子]拆成几步的那个模块]。\n"
        "1) 打比方：抽象概念用生活化比喻。如[世界模型] -> [让机器人在脑子里预先推演一遍动作结果]。\n"
